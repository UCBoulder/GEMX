...
// !         write(*,*)bdcurlbp
        dbdxp = wx0*wz0*dbdx_c(i,k)+wx0*wz1*dbdx_c(i,k+1) 
         ...
         kapnxp = wx0*wz0*captix_c(i,k)+wx0*wz1*captix_c(i,k+1) 
                 +wx1*wz0*captix_c(i+1,k)+wx1*wz1*captix_c(i+1,k+1); 

         kaptzp = wx0*wz0*captiz_c(i,k)+wx0*wz1*captiz_c(i,k+1) 
                 +wx1*wz0*captiz_c(i+1,k)+wx1*wz1*captiz_c(i+1,k+1); 
         kapnzp = wx0*wz0*captiz_c(i,k)+wx0*wz1*captiz_c(i,k+1) 
                 +wx1*wz0*captiz_c(i+1,k)+wx1*wz1*captiz_c(i+1,k+1); 
...
// !         w3(m)=w2(m) + 0.5*dt*(vxdum*kapxp + vzdum*kapzp+edot/ter)*dum*xnp
        ...
        if( !(x3_ptr[m]>2*dxeq) && (x3_ptr[m]<lx-2*dxeq) && (z3_ptr[m]>2*dzeq) && (z3_ptr[m]<lz-2*dzeq)) //slightly altered logic here
          u3_ptr[m]=u2_ptr[m];
          x3_ptr[m]=x2_ptr[m];
          z3_ptr[m]=z2_ptr[m];
