...
subroutine fourier_modes(input_phi, modes)
   ...
   logical :: fourier_mask(n_points/2 + 1)
   ...
   complex(8), dimension(:,:,:), allocatable :: phi_hat
   allocate(phi_hat(0:imx, 0:jmx, 0:kmx))

   allocate(xarray(1:500),testfunction(1:500),f_filtered(1:500),f_hat(1:500))
   ! allocate(plan_forward,plan_backward)

   ! Initialize phi_hat to zero
   phi_hat(:,:,:)= 0.0

   do i = 1, n_points
      xarray(i) = (i - 1) * 2.0 * pi / (n_points)
   end do

   do i = 1, n_points
      testfunction(i) = sin(1.0 * xarray(i)) * sin(2.0 * xarray(i)) * sin(3.0 * xarray(i)) * sin(4.0 * xarray(i)) * sin(5.0 * xarray(i))
      f_hat(i) = 0
   end do

   
   ! call dfftw_plan_dft_1d(plan_forward,kmx+1,input_phi(0,0,:),input_phi(0,0,:),FFTW_FORWARD,FFTW_MEASURE)
   ! call dfftw_plan_dft_1d(plan_backward,kmx+1,input_phi(0,0,:),input_phi(0,0,:),FFTW_BACKWARD,FFTW_MEASURE)
   
   call dfftw_plan_dft_1d(plan_forward,n_points,testfunction,f_hat,FFTW_FORWARD,FFTW_MEASURE)
   !call dfftw_plan_dft_1d(plan_backward,n_points,testfunction,FFTW_BACKWARD,FFTW_MEASURE)
   write(*,*) 'Ich bin da jetzt!'
   call dfftw_execute_dft(plan_forward,testfunction,f_hat)
   

   ! Normalize the Fourier coefficients
   do i = 1, n_points/2 + 1
      f_hat(i) = f_hat(i) / real(n_points, 8)
   end do

   ! Create a mask for the Fourier coefficients
   fourier_mask    = .true.
   fourier_mask(1) = .false. ! Keep the zero frequency
   fourier_mask(9) = .false. ! Keep the 8th mode

   ! Remove specific modes
   do i = 1, n_points/2 + 1
      if (.not. fourier_mask(i)) then
         f_hat(i) = 0.0
      end if
   end do
   
   call dfftw_plan_dft_1d(plan_backward,n_points,f_hat,f_filtered,FFTW_BACKWARD,FFTW_MEASURE)
   call dfftw_execute_dft(plan_backward,f_hat,f_filtered)

   

   ! Loop over radial and z indices
   ! do i = 0, imx
      ! do j = 0, jmx
         ! Perform forward FFT
         ! call dfftw_execute_dft(plan_forward,input_phi(i,j,:),input_phi(i,j,:))

         ! Zero out unwanted modes
         ! do k = 0, kmx
         !    if (.not. any(modes == k)) then
         !       phi_hat(i,j,k) = 0.0
         !    end if
         ! end do

         ! Perform inverse FFT to reconstruct the input_phi
         ! call dfftw_execute_dft(plan_backward,input_phi(i,j,:),input_phi(i,j,:))
      ! end do
   ! end do
   ...
   if(timestep==0)then
      ...
      open(unit=11, file = 'testfourier',status='unknown',action='write')
         do i=1,n_points
      
               write(11,*) f_filtered(i)
         enddo
